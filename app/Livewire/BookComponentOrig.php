...
class BookComponentOrig extends Component
{
    use WithFileUploads;
    ...
    public function save(BookService $repo) {
        $this->validate([
        ...
        if ($this->id == 0) {
            $book = $repo->createBook($this->only(['title','rating','year','description','category_id', 'image']));
            //$book = Book::create($this->only(['title','rating','year','description','category_id', 'image']));
            //$book->authors()->sync($this->book_authors);          
        } else {
            $book = $repo->updateBook($this->id, ['authors' => $this->book_authors, ...$this->only(['id', 'title','rating','year','description','category_id', 'image',])]);
